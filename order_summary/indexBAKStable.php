	...
	?>
        ...
        <div class="container">
			...
			<form class="form-2" ><center>
					<h1><span class="order_count">NEW ORDER: <?php	echo $bil = $arr1['bil'];
						if(@$_SESSION['count']!=$bil){
							$_SESSION['count'] = $bil;
							echo "<script>alert('New Order Alert!');</script>";
						}
						?> </span></h1><h3>Order Confirmation (Call/SMS/Whatsapp...)</h3></center>
					
					
				</form>​​
			<?php  while($arr = mysqli_fetch_array($res)){ ?>
				<form class="form-2" action="proses_order.php?order_id=<?php echo $arr['order_id'];	?>&status=2" method="post">
...
<input type="checkbox" name="status" value="delivery">Delivery Completed -->
						...
						<a href="proses_order.php?order_id=<?php echo $arr['order_id'];	?>&status=3" class="log-cancel">CANCELED</a> 
					</p>
				</form>​​
			<?php }	?>
			</section>
